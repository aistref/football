"""Stage 5 — analyse Run B 21 sep 2026. Beide methodes, alle zes markten, acht poorten.

Ongewijzigd ten opzichte van 11 sep, op één punt na: dit script draait **twee keer**. De eerste
keer zonder BTTS, omdat §1a stap 2 die markt pas laat kopen voor wedstrijden die "al een
kandidaat-edge tonen"; `ra19_btts.py` koopt daarna in en deze analyse draait opnieuw, nu met de
BTTS-selecties erbij. Dat is openstaand punt 4 uit `runs/2026-09-12-run-a.md` opgelost.

Verder als op 11 sep: 1X2 op de beste prijs uit de h2h-bulk (§1a, incl. `net_price` bij een
beurs), `my_raw` = 0.80 xG / 0.20 splits (§1f), `my_prob` = `recalibrate.apply(my_raw)` (§1g). Het
BetExplorer-gemiddelde blijft in gebruik voor het kalibratieblok (§6e) en poort 8: dat zijn
oordelen over de wedstrijd, geen prijzen om op te spelen.
"""
import json, sys
...
odds = json.load(open("tmp-run/rb21_odds.json"))
...
def league_ctx(comp, pid, season, season_cur=None):
    if comp not in league_cache:
        ...
        # Run B-specifiek (§4, datalabels): zes van de dertien competities van vandaag hebben
        # geen xG bij Fotmob. Dan is het competitieniveau het doelpuntgemiddelde — dezelfde
        # terugval als `b_analyze.py` van 19 sep. Die competities komen sowieso op LIGHT uit.
        avg = st["avg_xg_per_match"]
        if avg is None:
            avg = (st["home_goals_per_match"] + st["away_goals_per_match"]) / 2
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/rb21_results.json", "w"), ensure_ascii=False, indent=1)
