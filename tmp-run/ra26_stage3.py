"""Run A, 26 sep 2026 — Stage 1/2/3: fixtures via het inzetvenster, competitiepoort, bronprobe.

Gelijk aan `ra25_stage3.py`, met twee verschillen: de rundag is 26 september en
`include_carry_over` staat weer **uit**. Die vlag was op 25 september eenmalig bedoeld — hij is
er om de band [00:00, 08:00) NL van de overstapdag niet onbekeken te laten, en die band is
inmiddels door de run van 25 september meegenomen. Hem laten staan zou dezelfde wedstrijd in twee
runrapporten zetten (§3, Stage 1).
"""
import json
...
json.dump({"day": DAY.isoformat(), "carry_over": CARRY, "ids": ids, "id_source": id_source,
          ...
          open("tmp-run/ra26_stage3.json", "w"), ensure_ascii=False, indent=1)
