...
# (zie de toelichting in rb19_analyze.py). Zou deze controle de factor opnieuw uitrekenen op
...
_vs = json.load(open("tmp-run/rb19_results.json"))["vroeg_seizoen"]
...
if not used:
    ...
    json.dump({"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD, "n": 0,
              ...
              open("tmp-run/rb19_uplift.json", "w"), ensure_ascii=False, indent=1)
    raise SystemExit(0)
