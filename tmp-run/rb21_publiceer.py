...
CAPTURED = "2026-09-21T05:20:00+02:00"      # uitleestijd van het BetExplorer-marktgemiddelde
...
if __name__ == "__main__":
    alle = []
    for run, pad in (("B", "tmp-run/rb21_results.json"),):
        st, top, picks, res = bouw(run, pad)
        json.dump(top, open(f"tmp-run/rb21_top_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        json.dump(picks, open(f"tmp-run/rb21_picks_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        alle += picks
    ...
    for run, pad in (("B", "tmp-run/rb21_results.json"),):
        ...
        state["parameters"] = {
            "HERIJKING": ("recalibrate.apply, fit op uitslagen — a=1.030, b=+0.019 op 2346 "
                          ...
                          "vrijwel neutraal (+0.5 pp bij 50%) in plaats van de ~10 pp aftrek "
                          "van begin september."),
            ...
            "inkoop": ("0 credits uitgegeven: de enige competitie die vandaag speelt (Romanian "
                       "SuperLiga) heeft geen sportkey bij The Odds API, dus er was niets "
                       "inkoopbaars. 1X2 komt van het BetExplorer-marktgemiddelde; AH, DNB, DC, "
                       "OU en BTTS waren vandaag in het geheel niet te koop."),
