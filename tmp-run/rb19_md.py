"""Hulpstuk: dekkingstabel en wedstrijdsecties voor runs/2026-09-19-run-b.md uit run-state.
...
ST = json.load(open("data/run-state/2026-09-19-run-b.json"))
RES = {m["match"]: m for m in json.load(open("tmp-run/rb19_results.json"))["matches"]}
...
for comp, b in ST["competitions"].items():
    ...
    if st == "GEEN WEDSTRIJD":
        ...
        # §5 kent `AFGEKAPT` als aparte status. Vandaag viel de cap op twee duels die
        # sowieso al op `NONE` stonden, dus de status blijft staan wat hij is — maar
        # stille truncatie is verboden, dus het staat erbij.
        cap = [m["match"] for m in b["matches"] if m.get("buiten_cap")]
        if cap:
            toel += (f"; {len(cap)} buiten `MAX_DEEP_ANALYSES` gevallen "
                     f"({', '.join(cap)}) — al op `NONE`, dus geen analyse verloren")
...
open("tmp-run/rb19_coverage.md", "w").write("\n".join(cov))
open("tmp-run/rb19_matches.md", "w").write("\n".join(secs))
print("\n".join(cov))
print()
print("\n".join(secs))
