"""Hulpstuk: dekkingstabel en wedstrijdsecties voor runs/2026-09-17-run-a.md uit run-state.
...
ST = json.load(open("data/run-state/2026-09-17-run-a.json"))
RES = {m["match"]: m for m in json.load(open("tmp-run/ra17_results.json"))["matches"]}
...
open("tmp-run/ra17_coverage.md", "w").write("\n".join(cov))
open("tmp-run/ra17_matches.md", "w").write("\n".join(secs))
print("\n".join(cov))
print()
print("\n".join(secs))
