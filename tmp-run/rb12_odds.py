"""Run B, 12 sep 2026 — Stage 3 vervolg: prijzen inkopen (§1a, bulk met h2h erbij).

Overgenomen van `ra11_odds.py` van vanochtend; het enige Run B-verschil zit in de
inkoopbaarheid. Drie van de acht competities die vandaag spelen hebben **geen sportkey**
bij The Odds API — de Romanian SuperLiga, de Keuken Kampioen Divisie en de Kategoria
Superiore — dus daar is BetExplorer de enige 1X2-bron (marktgemiddelde, bookmaker niet
herleidbaar, §1a laatste alinea) en zijn AH/DNB/DC/OU/BTTS niet in te kopen. Dat is geen
creditplafond: er staat ruim 19.000 credits open. De vijf overige competities krijgen de
volle bulk-aanroep.
"""
import json, sys
...
DAY = date(2026, 9, 12)
REMAINING, DAYS_LEFT = 19386, 19    # uit api_check.py van deze run (614 gebruikt deze maand)
...
json.dump(store, open("tmp-run/rb12_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n" + guard.report())
