"""Stage 5 — analyse Run B 19 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
# Waarom een markt niet is opgevraagd. Dit is geen vaste tekst: hij wordt uit de werkelijke
# inkoop van deze run opgebouwd, zodat de reden nooit een overgebleven getal van gisteren is.
# Een markt die je door het plafond niet hebt opgevraagd is "bekeken met een reden" (§1a), en
# het verschil tussen "niet in te kopen" en "geen geld meer" is precies wat §6b-5b wil zien.
GEEN_KEY_REDEN = (
    "geen sportkey voor deze competitie bij The Odds API — niet in te kopen "
    "(coverage.json, 9 aug 2026); niet het creditplafond, dat stond op {cap} met {spent} "
    "uitgegeven, dus {over} ongebruikt")

FIT = recalibrate.load_fit()            # §1g — één keer per run, leest picks.jsonl + shadow.jsonl
print(f"herijking: {FIT}")

cands = json.load(open("tmp-run/rb19_ctx.json"))
s3 = json.load(open("tmp-run/rb19_stage3.json"))
stats_meta, us_meta = s3["stats"], s3.get("understat", {})   # Run B: geen Understat-competitie in de runlijst
odds = json.load(open("tmp-run/rb19h_odds.json"))
GEEN_KEY_REDEN = GEEN_KEY_REDEN.format(cap=odds["cap"], spent=odds["spent"],
                                       over=odds["cap"] - odds["spent"])

# ---------- vroeg-seizoenscorrectie (§3 Stage 5) -------------------------------------------
# Zes van de vijftien competities die vandaag spelen hebben géén xG bij Fotmob (Czech First
# League, Croatian HNL, Hungarian NB I, Romanian SuperLiga, Keuken Kampioen Divisie en
# Kategoria Superiore), dus voor die competities bestaat de gebruikelijke observatie `(avg_xg vorig, avg_xg dit,
# speeldagen)` niet. Ze stilzwijgend overslaan zou de correctie op de overige competities laten
# leunen; ze op 1.0 zetten is niet "geen correctie nodig" maar "niet gemeten".
#
# Wat er voor die competities in plaats daarvan in gaat: hetzelfde niveauverschil, gemeten op
# DOELPUNTEN. Dat is de eenheid waarin die duels sowieso rekenen (zie `league_ctx` hieronder:
# zonder xG komt zowel het competitieniveau als de teamsterkte uit doelpunten), dus teller en
# noemer staan in dezelfde eenheid. Het blijft een waarneming uit de stand en er komt geen enkele
# bookmakerprijs aan te pas — §2 en de waarschuwing bij `early_season_uplift` blijven dus
# gerespecteerd. De gepoolde factor loopt vandaag over vijftien competities en is daarmee de
# ruimste steekproef die deze routine tot nu toe heeft gehad.
...
def league_ctx(comp, pid, season, season_cur=None):
    if comp not in league_cache:
        ...
        # Zes van de vijftien competities die vandaag spelen hebben geen xG bij Fotmob. Het
        # competitieniveau komt daar uit de doelpunten zelf; de teamsterktes hieronder doen
        # hetzelfde, zodat teller en noemer in dezelfde eenheid staan.
...
for c in cands:
    ...
    row = {"competition": c["competition"], "match": f"{c['home']} – {c['away']}",
           ...
           "context": c.get("ctx")}
    # Stille truncatie is verboden (§3 Stage 4). De `afgekapt`-vlag hieronder wordt pas gezet
    # nadat de tier-poorten zijn gepasseerd, dus een duel dat zowel buiten de cap valt als op
    # NONE uitkomt verloor die vermelding. Deze vlag staat er altijd, ongeacht welke uitgang
    # de rij neemt, zodat per wedstrijd terug te zien is of de cap hem heeft geraakt.
    if id(c) not in keep:
        row["buiten_cap"] = True
    ...
    elif not c.get("sportkey"):
        row["markets_checked"]["BTTS"] = GEEN_KEY
    elif c["match_id"] in odds["bought"]["btts"]:
        row["markets_checked"]["BTTS"] = "btts opgevraagd, maar geen boek noteerde deze markt"
    else:
        # Sinds vandaag koopt de run BTTS pas in de tweede ronde, voor de duels met een
        # kandidaat-edge (§1a stap 2). Voor alle andere duels is de markt dus niet opgevraagd,
        # en dat is iets anders dan "opgevraagd en niet genoteerd". §6b-5b wil precies dat
        # verschil kunnen zien; de oude tekst zou hier van 24 duels beweren dat er een lege
        # respons kwam terwijl er nooit een verzoek is gedaan.
        row["markets_checked"]["BTTS"] = (
            "niet opgevraagd — geen kandidaat-edge op de ruwe of de herijkte schaal "
            "(§1a stap 2); niet het creditplafond, dat stond op "
            f"{odds['cap']} met {odds['spent']} uitgegeven")
    ...
    if _p8:
        ...
        # 13 sep 2026 — §5 eist in de "Net niet"-tabel alle drie de getallen, en juist bij een
        # poort-8-rij is `edge_robust_min` per definitie leeg: `robustness_check` draait hierboven
        # alleen als *alle* basispoorten openstaan en poort 8 zit daarin. Eén aanroep per
        # wedstrijd. Deze regel stond wel in het Run A-script en niet in dat van Run B.
    ...
    # §1a stap 2 — welke duels een "kandidaat-edge" tonen en dus BTTS waard zijn. Sinds 13 sep
    # 2026 is dat de VERENIGING van beide schalen: de herijkte NEAR-drempel (de grens waarboven
    # een selectie als `near_miss` het schaduwlogboek ingaat) óf de volle drempel op de ruwe
    # schaal (de grens die `zonder_herijking` vult, en dus de reeks waarmee §5a de herijking
    # beoordeelt). Van 6 t/m 12 sep gebruikte Run B alleen de eerste lezing en kocht BTTS
    # bovendien vooraf voor de bovenste `MAX_DEEP_ANALYSES` duels; dan is "toont al een
    # kandidaat-edge" geen criterium maar een rangschikking. De vereniging is nooit krapper.
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/rb19h_results.json", "w"), ensure_ascii=False, indent=1)
