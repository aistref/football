"""Stage 5 — analyse Run A 8 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
cands = json.load(open("tmp-run/ra8_ctx.json"))
...
for c in cands:
    ...
    # Kruis-grens (§4). De twee ploegen komen uit verschillende NATIONALE competities en de
    # sterktes zijn genormaliseerd op hun eigen competitiegemiddelde. Het krachtsverschil
    # tussen die twee competities kent het model niet: promotion.MEASURED_TIER2_GAP meet
    # divisies BINNEN een land, en tussen twee landen bestaat zo'n gemeten factor niet.
    # Doorrekenen levert dan geen kansschatting maar de onbekende competitiesterkte terug —
    # op 19 aug 2026 zeventien procentpunt op één uitwinst. Dus: NONE, geen bet, en géén
    # near_miss, want een cijfer dat we zelf onbruikbaar noemen hoort niet in het logboek
    # waarmee de poorten worden beoordeeld (§6d).
    if c.get("crossborder"):
        row["tier"] = "NONE"
        row["context"] = c.get("ctx")
        row["reason"] = ("kruis-grens — ploegen uit verschillende nationale competities, geen "
                         "gemeten omrekenfactor tussen die competities (§4)")
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra8_results.json", "w"), ensure_ascii=False, indent=1)
