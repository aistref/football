...
W("**Leesbaar dagrapport:** https://claude.ai/code/artifact/b860ad26-e36c-4d8a-a38e-2cce716f9ef7\n")

# ---------------------------------------------------------------- looptijd
W("""## Looptijd — en waarom het getal in `duur` misleidt

§0 laat elke run sinds 5 september zijn looptijd vastleggen, met één doel: kunnen zien of een cap
van 55 vóór 06:30 haalbaar is in plaats van het te moeten raden. Vandaag is de eerste Run B-dag
waarop die vraag echt speelt, dus hier het antwoord met de bestandstijden erbij.

| Stap | Klaar om | Bestand |
|---|---|---|
| gestart | 05:10:30 | — |
| fixtures, competitiepoort, bronprobe | 05:12:33 | `tmp-run/rb12_stage3.json` |
| context + datarijkdom, **alle 59 duels** | 05:15:55 | `tmp-run/rb12_ctx.json` |
| prijzen ingekocht (10 bulk + 46 BTTS) | 05:16:40 | `tmp-run/rb12_odds.json` |
| analyse, 45 duels en 641 selecties | 05:16:58 | `tmp-run/rb12_results.json` |
| leesbaar dagrapport | 05:26:45 | `runs/2026-09-12-run-b.html` |

**Negenenvijftig duels, beide rapporten klaar binnen zeventien minuten** — ruim een uur vóór de
06:30-grens. De rekentijd is dus niet de beperking geweest; de cap was dat.

`duur.minuten` staat in het voortgangsbestand desondanks op **157,9**, en dat getal moet je hier
niet voor gebruiken. Het is wandkloktijd: de sessie is ná het publiceren van het dagrapport
onderbroken en pas om 07:48 hervat om het voortgangsbestand af te sluiten en te pushen. Er staat
een `duur.noot` bij die dit uitlegt, want anders leest de volgende run dit als "55 past niet" —
precies het omgekeerde van wat er gemeten is.

**Wat dit wél zegt over `MAX_DEEP_ANALYSES`.** Vandaag kostte de cap drie analyses op een dag die
in zeventien minuten rond was. Op deze cijfers is 55 ruim aan de veilige kant. Dat is één
waarneming en geen voorstel — de cap is op 5 september op een meting verhoogd en hoort op een
meting te bewegen, niet op één drukke zaterdag — maar het is wel het soort waarneming waarvoor §0
dit veld heeft laten aanleggen.
""")
