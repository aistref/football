"""Hulpstuk: dekkingstabel en wedstrijdsecties voor runs/2026-09-18-run-a.md uit run-state.
...
ST = json.load(open("data/run-state/2026-09-18-run-a.json"))
RES = {m["match"]: m for m in json.load(open("tmp-run/ra18_results.json"))["matches"]}
...
open("tmp-run/ra18_coverage.md", "w").write("\n".join(cov))
open("tmp-run/ra18_matches.md", "w").write("\n".join(secs))
print("\n".join(cov))
print()
print("\n".join(secs))
