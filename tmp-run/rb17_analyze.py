"""Stage 5 — analyse Run B 17 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
kandidaat-edge tonen"; `rb17_btts.py` koopt daarna in en deze analyse draait opnieuw, nu met de
...
cands = json.load(open("tmp-run/rb17_ctx.json"))
...
for c in cands:
    ...
    if not crossborder:
      ...
      # ónder de basisdivisie, dan zet `rb17_cand.py` die divisie hier neer en wordt het duel
      ...
      def side_stats(name, table_key):
          ...
          # De runlijst van Run B schrijft "English League One (ENG)" en "Segunda División (ESP)";
          # TIER1/TIER2 kennen die divisies onder hun eigen naam ("League One (ENG)", "LaLiga2
          # (ESP)"). `promotion._resolve` is precies daarvoor gemaakt — zie de docstring bij
          # COMPETITION_ALIASES: "Vertaal daarom altijd via `_resolve` en niet met een rauwe
          # TIER2.get(naam)". Zonder deze regel valt een promovendus uit League Two (vandaag MK
          # Dons) ten onrechte op NONE, terwijl E2/E3 gewoon gemeten in MEASURED_GAPS staat.
          pcomp = promotion._resolve(pcomp)
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/rb17_results.json", "w"), ensure_ascii=False, indent=1)
