"""Stage 5 — analyse Run A 16 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
kandidaat-edge tonen"; `ra16_btts.py` koopt daarna in en deze analyse draait opnieuw, nu met de
...
for c in cands:
    ...
    if not crossborder:
      # `basis_comp` (16 sep 2026): staan beide ploegen van een bekerduel in dezelfde divisie
      # ónder de basisdivisie, dan zet `ra16_cand.py` die divisie hier neer en wordt het duel
    ...
    # Poort 8 zoals hij op de RUWE schaal bindt (toegevoegd 16 sep 2026, openstaand punt 2).
    #
    ...
    if passed:
        ...
        # dat is een bewuste keuze van 16 sep 2026. `near_miss` voedt twee dingen: het
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra16_results.json", "w"), ensure_ascii=False, indent=1)
