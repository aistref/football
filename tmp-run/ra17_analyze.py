"""Stage 5 — analyse Run A 17 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
kandidaat-edge tonen"; `ra17_btts.py` koopt daarna in en deze analyse draait opnieuw, nu met de
...
cands = json.load(open("tmp-run/ra17_ctx.json"))
...
for c in cands:
    ...
    if not crossborder:
      ...
      # ónder de basisdivisie, dan zet `ra17_cand.py` die divisie hier neer en wordt het duel
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra17_results.json", "w"), ensure_ascii=False, indent=1)
