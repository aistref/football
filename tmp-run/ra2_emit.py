...
CAPTURED = "2026-09-02T04:20:00+02:00"

res = json.load(open("tmp-run/ra2_results.json"))
odds = json.load(open("tmp-run/ra2_odds.json"))
s3 = json.load(open("tmp-run/ra2_stage3.json"))
...
state["parameters"] = {
    ...
    "toelichting": (
        "14 wedstrijden op de runlijst vandaag, in 6 van de 21 competities — woensdag van de "
        "interlandbreak, de overige 15 hadden niets op de kalender. Championship 4, "
        "Scottish Premiership 5, Belgian Pro League 1, Danish Superliga 1, Coppa Italia 2, "
        "DFB Pokal 1. Dertien haalden de datadekkingspoort en zijn doorgerekend; VfL Osnabrück "
        "speelt in de 3. Liga en staat niet in de 2. Bundesliga-stand waarvoor de omrekening "
        "gemeten is, dus dat duel blijft op NONE. "
        f"De cap van {res['afkapping']['cap']} is niet aangeraakt, dus er is niets afgekapt."),
    "omrekeningen": {
        "aanleiding": (
            "Vier van de dertien doorgerekende duels hebben een ploeg die vorig seizoen niet in de "
            "stand van de competitie van vandaag stond: Cardiff City (League One -> Championship), "
            "Burnley (Premier League -> Championship), St. Johnstone (Championship SCO -> "
            "Premiership), en Frosinone en Venezia (Serie B, Coppa Italia). Alle vijf zijn omgerekend "
            "en dus LIGHT, nooit FULL."),
        "naamkoppeling": (
            "QPR en West Bromwich Albion gingen bij de eerste doorloop ten onrechte de omrekening in: "
            "'QPR' is een initiaalwoord dat geen token deelt met 'Queens Park Rangers', en bij "
            "'West Brom' valt 'albion' weg in _DROP zodat 'brom' en 'bromwich' overblijven. Beide "
            "staan gewoon in de Championship-stand van 2025/2026. Twee aliassen toegevoegd; beide "
            "duels zijn daarna als FULL doorgerekend."),
        "toegepast_op": ["Cardiff City (up, E1/E2)", "Burnley (down, E0/E1)",
                         "St. Johnstone (up, SC0/SC1)", "Frosinone (up, I1/I2)",
                         "Venezia (up, I1/I2)"],
        "geweigerd": ["VfL Osnabrück — speelt in de 3. Liga, niet in de 2. Bundesliga-stand die "
                      "promotion.TIER2 voor Duitsland kent"],
    },
...
state["credits"] = {
    ...
    "bron": ("suggest_cap(19961, 29) — 19.961 credits over volgens api_check.py van vanochtend "
             "(20K-plan, 39 gebruikt deze maand), 29 dagen tot de maandwissel, 2 runs per dag"),
...
json.dump({"short": sorted(short), "bets": [m["match"] for m in bets]},
          open("tmp-run/ra2_short.json", "w"), ensure_ascii=False)
