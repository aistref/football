"""Run B 20 sep 2026 onder §5b: run-state, picks en logboeken.
...
CAPTURED = "2026-09-20T05:05:00+02:00"      # uitleestijd van de bulk-aanroep
...
if __name__ == "__main__":
    alle = []
    for run, pad in (("B", "tmp-run/rb20_results.json"),):
        st, top, picks, res = bouw(run, pad)
        json.dump(top, open(f"tmp-run/rb20_top_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        json.dump(picks, open(f"tmp-run/rb20_picks_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        alle += picks
...
def wegschrijven():
    ...
    for run, pad in (("B", "tmp-run/rb20_results.json"),):
        ...
        state["parameters"] = {
            ...
            "eerste_run_onder_5b": ("Eerste Run B die §5b van meet af aan draait: de drempel van "
                                    "8.0/16.0 pp is hier geen poort vooraf meer maar een afkapping "
                                    "aan het eind van de rangorde over alle 43 duels."),
            "p_xg_shrink08": ("Vanaf deze run vastgelegd, zoals §6e sinds 19 sep eist. Run A heeft "
                              "dat vandaag niet gedaan, waardoor calibration.py stats de "
                              "shrink-arm met zichzelf vergelijkt. Zie het runrapport, Bevinding 2."),
        }
