"""Stage 5 — analyse Run A 9 sep 2026. Beide methodes, alle zes markten, acht poorten.

Verschil met 8 sep: de Champions League-duels worden doorgerekend in plaats van blind op NONE
gezet. `scripts/interleague.py` (§4 "Kruis-grens", 8 sep 2026) rekent beide ploegen om naar de
Europese schaal met een op 2111 Europese duels gemeten aanval- en verdedigingsfactor per
nationale competitie; ze komen dan tegenover elkaar in `interleague.reference_league()`. Een
omgerekende ploeg is nooit FULL, dus die duels staan op LIGHT (drempel 16.0 pp), en zonder
gemeten factor of buiten het gemeten bereik blijft het NONE.

Ongewijzigd sinds 5 sep: 1X2 op de beste prijs uit de h2h-bulk (§1a, incl. `net_price` bij een
beurs), `my_raw` = 0.80 xG / 0.20 splits (§1f), en `my_prob` = `recalibrate.apply(my_raw)` (§1g).
Het BetExplorer-gemiddelde blijft in gebruik voor het kalibratieblok (§6e) en poort 8: dat zijn
...
cands = json.load(open("tmp-run/ra11_ctx.json"))
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra11_results.json", "w"), ensure_ascii=False, indent=1)
