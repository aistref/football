...
LEAGUES = {
 ...
 "Kategoria Superiore (ALB)":     (260, "ALB", "2025/2026", "2026/2027",
                                   ...
                                   None),
 # Kosovo staat niet in betexplorer.KNOWN_LEAGUE_URLS en heeft geen sportkey bij The Odds API.
 # De Fotmob-id wordt hieronder uit de daglijst opgezocht in plaats van geraden.
 "Kosovo Superleague (KOS)":      (None, "KOS", "2025/2026", "2026/2027", None, None),
}

fx = fotmob.fetch_fixtures(DAY)

# Kosovo: id opzoeken in de daglijst van vandaag (nooit raden — §2, "niet gevonden = niet gevonden")
kos = [lg for lg in fx.get("leagues", []) if lg.get("ccode") == "KOS"]
if kos:
    print("KOS in de daglijst:", [(l.get("primaryId") or l.get("id"), l.get("name"),
                                   len(l.get("matches", []))) for l in kos])
...
for name, (lid, cc, s_prev, s_cur, bx, key) in LEAGUES.items():
    ...
    if lg is None and cc == "KOS":
        # Kosovo heeft maar één competitie in de daglijst; pak die als hij er is.
        lg = next((l for l in kos if l.get("matches")), None)
        if lg is not None:
            lid = lg.get("primaryId") or lg.get("id")
            print(f"Kosovo Superleague (KOS): Fotmob-id {lid} gevonden in de daglijst")
...
json.dump({"fixtures": out, "stats": stats, "understat": {}},
          open("tmp-run/rb20_stage3.json", "w"), ensure_ascii=False, indent=1)
