...
kandidaat-edge tonen"; `ra18_btts.py` koopt daarna in en deze analyse draait opnieuw, nu met de
...
cands = json.load(open("tmp-run/ra18_ctx.json"))
...
for c in cands:
    ...
    if not crossborder:
      ...
      # ónder de basisdivisie, dan zet `ra18_cand.py` die divisie hier neer en wordt het duel
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra18_results.json", "w"), ensure_ascii=False, indent=1)
