...
cands = json.load(open("tmp-run/rb9_cands.json"))
stats = json.load(open("tmp-run/rb9_stage3.json"))["stats"]
out = []
for c in cands:
    if c["tier"] == "NONE":
...
json.dump(out, open("tmp-run/rb9_ctx.json", "w"), ensure_ascii=False, indent=1, default=str)
