"""Stage 5 — analyse Run A 20 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
cands = json.load(open("tmp-run/ra20_ctx.json"))
s3 = json.load(open("tmp-run/ra20_stage3.json"))
stats_meta, us_meta = s3["stats"], s3["understat"]
odds = json.load(open("tmp-run/ra20_odds.json"))
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/ra20_results.json", "w"), ensure_ascii=False, indent=1)
