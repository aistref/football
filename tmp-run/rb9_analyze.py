"""Stage 5 — analyse Run B 9 sep 2026. Beide methodes, alle zes markten, acht poorten.
...
for c in cands:
    ...
    GEEN_KEY = ("geen sportkey voor deze competitie bij The Odds API — niet in te kopen "
                "(coverage.json, 9 aug 2026); niet het creditplafond, dat stond op 443 ongebruikt")
...
json.dump({"vroeg_seizoen": {"factor": FACTOR, "gepoold": POOLED, "speeldagen": TOTAL_MD,
          ...
          open("tmp-run/rb9_results.json", "w"), ensure_ascii=False, indent=1)
