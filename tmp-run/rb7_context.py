...
cands = json.load(open("tmp-run/rb7_cands.json"))
stats = json.load(open("tmp-run/rb7_stage3.json"))["stats"]
out = []
for c in cands:
    if c["tier"] == "NONE":
...
json.dump(out, open("tmp-run/rb7_ctx.json", "w"), ensure_ascii=False, indent=1, default=str)
