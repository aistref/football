"""Herdraai 19 sep 2026 onder §5b: run-state, picks en logboeken voor beide runs.
...
CAPTURED = "2026-09-19T11:05:00+02:00"      # prijzen van de herdraai
...
if __name__ == "__main__":
    ...
    for run, pad in (("A", "tmp-run/ra19h_results.json"), ("B", "tmp-run/rb19h_results.json")):
        ...
        json.dump(top, open(f"tmp-run/h_top_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        json.dump(picks, open(f"tmp-run/h_picks_{run.lower()}.json", "w"), ensure_ascii=False, indent=1)
        alle += picks
