"""Run B, 16 sep 2026 — Stage 3 vervolg: prijzen inkopen (§1a, bulk met h2h erbij).

BTTS staat hier NIET bij: sinds 13 sep 2026 koopt `rb16_btts.py` die markt in een tweede
ronde, ná de eerste analyse, voor de duels met een kandidaat-edge op de ruwe OF de herijkte
schaal (§1a stap 2).

Vandaag spelen er uit de hele Run B-runlijst drie duels in twee competities: één in de
Allsvenskan en twee in de Swiss Super League. Beide hebben een sportkey bij The Odds API
(`soccer_sweden_allsvenskan`, `soccer_switzerland_superleague`), dus er gaan twee volle
bulk-aanroepen uit (h2h + spreads + totals, 3 credits per competitie) en staat 1X2 op de beste
prijs. BetExplorer levert daarnaast gratis het marktgemiddelde, dat alleen voor het
kalibratieblok (§6e) en poort 8 wordt gebruikt.
"""
import json, sys
...
DAY = date(2026, 9, 16)
REMAINING, DAYS_LEFT = 19140, 15    # uit api_check.py van deze run (860 gebruikt deze maand)
...
json.dump(store, open("tmp-run/rb16_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n" + guard.report())
