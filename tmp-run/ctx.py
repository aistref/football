...
L = json.load(open("tmp-run/leagues.json"))
cur_tables={}
for comp,blk in M.items():
    st = fotmob.fetch_league_stats(blk["league_id"], L[comp]["cur_season"])
...
for comp, blk in M.items():
    for m in blk["matches"]:
        ...
        try:
            ...
            print(f"OK  {comp[:20]:22s} {key[:36]:38s} rich={rich.total:5.1f} lineup={ctx.lineup_type or '-':10s} reloc={ctx.venue.relocated} g7h={rec['gate7']['home']['passed']} g7a={rec['gate7']['away']['passed']}")
            ...
            print(f"ERR {comp[:20]:22s} {key[:36]:38s} {type(e).__name__}: {e}  -> rich={rich.total:.1f} (middenpunt)")
        out[key]=rec
json.dump(out, open("tmp-run/context.json","w"), ensure_ascii=False, indent=1, default=str)
