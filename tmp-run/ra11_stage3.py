"""Run A, 8 sep 2026 — Stage 1/2/3: fixtures, competitiepoort, bronprobe + datadekking.

Verschil met 7 sep: de runlijst bevat vandaag twee toernooien met wedstrijden.
  * League Cup (ENG) — ploegen uit meerdere Engelse divisies. De basis (niveau + splits) is
    de Premier League 2025/2026; ploegen van buiten die divisie worden omgerekend door
    promotion.convert / convert_relegated, precies zoals bij de Coppa Italia op 1 sep.
  * UEFA Champions League — de ploegen komen uit verschillende landen. Er bestaat geen
    gemeten omrekenfactor tussen twee nationale competities (promotion.MEASURED_TIER2_GAP
    gaat over divisies BINNEN een land), dus het model kent het krachtsverschil tussen die
    competities niet. Die duels krijgen `crossborder = True` en gaan naar data_tier NONE.
"""
import json
...
# Toernooien waarin ploegen uit verschillende LANDEN tegen elkaar staan. Er is geen gemeten
# factor tussen twee nationale competities, dus het krachtsverschil is onbekend: data_tier NONE
# (§4 — geen onafhankelijke kansinput op het niveau waarop gespeeld wordt).
...
json.dump({"fixtures": out, "stats": stats, "understat": us_data},
          open("tmp-run/ra11_stage3.json", "w"), ensure_ascii=False, indent=1)
