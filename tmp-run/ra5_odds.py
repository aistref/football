...
5 sep 2026: 55 wedstrijden op de runlijst tegen twaalf gisteren. Spreads en totals kosten
1 credit per competitie en zijn dus even duur als altijd, maar BTTS kost 2 credits **per
wedstrijd** — voor alle 55 zou dat 110 credits zijn, waarvan er twintig naar duels gaan die
`MAX_DEEP_ANALYSES` sowieso afkapt. BTTS wordt daarom alleen gekocht voor de duels die op
(datakwaliteit, datarijkdom, aftrap) binnen de cap vallen; de markten-tie-break uit
`ranking.sort_key` staat achter die twee en kan de kop van de lijst niet meer omgooien.
"""
import json, sys
...
DAY = date(2026, 9, 5)
REMAINING, DAYS_LEFT = 19844, 26    # uit api_check.py van vanochtend (156 gebruikt deze maand)

cands = json.load(open("tmp-run/ra5_ctx.json"))
comps = []
for c in cands:
...
# Welke duels mogen BTTS kosten: de bovenste MAX_DEEP_ANALYSES op datakwaliteit en datarijkdom.
...
print(f"BTTS alleen voor de bovenste {DEEP} van {len(ranked)} duels met dekking")
...
json.dump(store, open("tmp-run/ra5_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n", guard.report())
