...
`ra19_analyze.py`): nooit krapper dan voorheen, en de reeks van §5a wordt er scherper van.

Het plafond van `ra19_odds.py` wordt hier opnieuw opgebouwd met het al uitgegeven bedrag erin,
...
store = json.load(open("tmp-run/ra20_odds.json"))
res = json.load(open("tmp-run/ra20_results.json"))
cands = {c["match_id"]: c for c in json.load(open("tmp-run/ra20_ctx.json"))}
...
json.dump(store, open("tmp-run/ra20_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n" + str(guard.report()))
