"""Run B, 15 sep 2026 — Stage 3 vervolg: prijzen inkopen (§1a, bulk met h2h erbij).

BTTS staat hier NIET bij: sinds 13 sep 2026 koopt `rb15_btts.py` die markt in een tweede
ronde, ná de eerste analyse, voor de duels met een kandidaat-edge op de ruwe OF de herijkte
schaal (§1a stap 2).

Vandaag speelt er uit de hele Run B-runlijst precies één duel: Grasshopper - Sion in de Swiss
Super League. Die competitie heeft wél een sportkey bij The Odds API
(`soccer_switzerland_superleague`), dus er gaat één volle bulk-aanroep uit (h2h + spreads +
totals, 3 credits) en staat 1X2 op de beste prijs. BetExplorer levert daarnaast gratis het
marktgemiddelde, dat alleen voor het kalibratieblok (§6e) en poort 8 wordt gebruikt.
"""
import json, sys
...
DAY = date(2026, 9, 15)
REMAINING, DAYS_LEFT = 19167, 16    # uit api_check.py van deze run (833 gebruikt deze maand)
...
json.dump(store, open("tmp-run/rb15_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n" + guard.report())
