...
store = json.load(open("tmp-run/ra19_odds.json"))
res = json.load(open("tmp-run/ra19_results.json"))
cands = {c["match_id"]: c for c in json.load(open("tmp-run/ra19_ctx.json"))}
...
json.dump(store, open("tmp-run/ra19_odds.json", "w"), ensure_ascii=False, indent=1)
print("\n" + str(guard.report()))
