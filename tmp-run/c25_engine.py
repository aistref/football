"""Kernfuncties voor Run C, 25 sep 2026 — herbruikbaar vanuit het hoofdscript."""
import math, sys
sys.path.insert(0, '.')
...
def nl_candidates(event, home_name, away_name, disp_home=None, disp_away=None):
    """Lijst van marktkandidaten (dict) uit één Odds-API event: 1X2, DNB, DC, AH, OU (geen BTTS).

    `home_name`/`away_name` moeten de team namen zijn ZOALS The Odds API ze teruggeeft
    (`event['home_team']`/`event['away_team']`) — matchen gaat daarop, niet op Fotmob-namen.
    Fotmob wijkt soms af (Turkiye/Turkey, Bosnia and Herzegovina/Bosnia & Herzegovina); zonder
    deze scheiding valt zo'n kant stilzwijgend weg uit alle markten (1X2, spreads, totals) omdat
    de naamvergelijking nooit matcht. `disp_home`/`disp_away` zijn de namen voor de leesbare
    selectielabel (Fotmob-namen, voor consistentie met de rest van het runrapport).
    """
    ...
    seen_points = set((pt) for (_, pt) in spreads.keys())
...
def bx_candidates(row, home_name, away_name):
    o1, ox, o2 = row["odds"]
    src = f"BetExplorer marktgemiddelde ({row.get('bookmakers', 0)} boeken)"
    return [
