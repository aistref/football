"""Hulpstuk: dekkingstabel en wedstrijdsecties voor runs/2026-09-19-run-a.md uit run-state.
...
ST = json.load(open("data/run-state/2026-09-19-run-a.json"))
RES = {m["match"]: m for m in json.load(open("tmp-run/ra19_results.json"))["matches"]}
...
for comp, b in ST["competitions"].items():
    ...
    if st == "GEEN WEDSTRIJD":
        ...
        cut = [m["match"] for m in b["matches"] if m.get("afgekapt")]
        if cut:
            toel += (f"; **{len(cut)}× `AFGEKAPT`** buiten MAX_DEEP_ANALYSES "
                     f"({', '.join(cut)})")
...
open("tmp-run/ra19_coverage.md", "w").write("\n".join(cov))
open("tmp-run/ra19_matches.md", "w").write("\n".join(secs))
print("\n".join(cov))
print()
print("\n".join(secs))
