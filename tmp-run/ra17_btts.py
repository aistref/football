...
`ra17_analyze.py`): nooit krapper dan voorheen, en de reeks van §5a wordt er scherper van.

Het plafond van `ra17_odds.py` wordt hier opnieuw opgebouwd met het al uitgegeven bedrag erin,
...
store = json.load(open("tmp-run/ra17_odds.json"))
res = json.load(open("tmp-run/ra17_results.json"))
cands = {c["match_id"]: c for c in json.load(open("tmp-run/ra17_ctx.json"))}
