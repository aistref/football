...
cands = json.load(open("tmp-run/rb18_cands.json"))
stats = json.load(open("tmp-run/rb18_stage3.json"))["stats"]
out = []
for c in cands:
    ctx = err = None
    try:
...
json.dump(out, open("tmp-run/rb18_ctx.json", "w"), ensure_ascii=False, indent=1, default=str)
