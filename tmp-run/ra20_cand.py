"""Stage 3 vervolg: tier per wedstrijd, met promovendi-, degradanten- en kruis-grensomrekening.
...
for name, v in fx.items():
    ...
    for m in v["matches"]:
        ...
        #
        # 18 sep 2026 — en de regel geldt alleen voor BEKERS. §4 verantwoordt hem met "een beker
        # heeft zelf geen stand en dus geen competitiegemiddelde om ploegen op te normaliseren";
        # een competitie heeft dat wél. Vandaag stonden twee Ekstraklasa-ploegen (Wisła Kraków en
        # Śląsk Wrocław) allebei niet in de Ekstraklasa-stand van 2025/2026 omdat ze allebei uit
        # de I Liga zijn gepromoveerd. De oude, competitiebrede lezing rekende hun duel dáár door
        # — op het doelpuntenniveau van de divisie eronder, terwijl ze vanavond in de Ekstraklasa
        # spelen — en liep bovendien vast omdat de I Liga geen xG heeft. Twee promovendi horen
        # gewoon allebei door `promotion.convert` naar het niveau waarop ze spelen (§4,
        # "Promovendi: eerst omrekenen, dan pas NONE").
        if len(missing) == 2 and name in promotion.CUP_BASE:
...
json.dump(cands, open("tmp-run/ra20_cands.json", "w"), ensure_ascii=False, indent=1)
