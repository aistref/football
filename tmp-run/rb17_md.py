"""Hulpstuk: dekkingstabel en wedstrijdsecties voor runs/2026-09-17-run-b.md uit run-state.
...
ST = json.load(open("data/run-state/2026-09-17-run-b.json"))
RES = {m["match"]: m for m in json.load(open("tmp-run/rb17_results.json"))["matches"]}
...
open("tmp-run/rb17_coverage.md", "w").write("\n".join(cov))
open("tmp-run/rb17_matches.md", "w").write("\n".join(secs))
print("\n".join(cov))
print()
print("\n".join(secs))
